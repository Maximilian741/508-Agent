...
def enforce_document_access(
    ...
    if not doc_id:
        ...
        # If a job_id is present we MUST resolve an owned document, otherwise we
        # deny (fail closed) rather than allowing on a valid session alone.
        job_id = request.path_params.get("job_id")
        if job_id:
            job = REPO.get_job(str(job_id))
            doc_id = (job or {}).get("docId")
            if not doc_id:
...
# The legacy /documents flow is a separate implementation (its own
# _analyze_*/_apply_*_fixes per format) that only supports Office + PDF. HTML is
# a first-class format in the MODERN /pipeline flow (audit + batch screens), so
# it is deliberately NOT accepted here — uploading HTML belongs on the audit
# screen. Keeping this list to 3 formats keeps the legacy path honest.
...
def _validate_upload_header(filename: str, content_type: str) -> str:
    ...
    if suffix not in _ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=400, detail="Unsupported file type. Only PDF, DOCX, and PPTX are allowed here — upload HTML on the Audit screen.")
...
# ---------------------------------------------------------------------------
# Retired: legacy fix / finalize / AI-review / fixed-download routes (410 Gone)
# ---------------------------------------------------------------------------
# These wrote fixes into the uploaded file (a title guessed from the filename,
# a hard-coded en-US language, placeholder form labels) and served the result
# with NO credit check and no rate limit; apply-fixes and ai-review also sent
# document bytes straight to OpenAI, outside SemanticInferenceClient's per-job
# cost cap. A zero-balance account got the paid deliverable for free.
#
# Documents are remediated only through POST /pipeline/remediate, which prices
# the job before any work, charges only for fixes that persist into the output
# bytes, and never writes a filename-guessed title or an assumed language (see
# smoke_doc_metadata_honesty). These routes answer 410, after the router's auth
# and ownership guard, so an old client fails with a pointer to the replacement.
# Upload, scan, issues, summary and the ORIGINAL download are read-only and stay.

LEGACY_REMEDIATION_RETIRED = (
    "This legacy fix and download endpoint has been retired. Fix documents on the "
    "Audit screen (POST /pipeline/remediate), where each fix is priced before you pay."
)


def _retired() -> HTTPException:
    return HTTPException(status_code=410, detail=LEGACY_REMEDIATION_RETIRED)


@router.post("/documents/{doc_id}/ai-review")
async def ai_review(doc_id: str) -> Dict[str, object]:
    raise _retired()


@router.post("/documents/{doc_id}/apply-fixes")
async def apply_fixes(doc_id: str) -> Dict[str, object]:
    raise _retired()
...
async def download_document(doc_id: str, variant: Optional[str] = "original"):
    ...
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
    if variant == "fixed":
        # Remediated output comes only from the charged /pipeline flow; this
        # route serves the caller's own upload and nothing else.
        raise _retired()
...
async def head_pdf_fixed(doc_id: str):
    raise _retired()


@router.get("/documents/{doc_id}/file-fixed")
async def download_fixed_file(doc_id: str):
    raise _retired()
...
async def head_pdf_rebuilt(doc_id: str):
    raise _retired()
