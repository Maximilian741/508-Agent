...
function _buildReportHtml(
  ...
  const branding = loadBranding(); // white-label branding for the deliverable
  ...
  <div class="page">
    ...
    <div class="body">
      ...
      <div class="conformance-callout">
        ...
        <strong>Automated checks performed:</strong> image alt text (incl. filename / placeholder alt that says nothing, 1.1.1), heading structure (Word &amp; PowerPoint; incl. long Word documents with no headings and text merely styled as a heading, 2.4.6 / 1.3.1), table headers (Word &amp; PowerPoint), list structure (Word &amp; PowerPoint — incl. typed "- item" lines converted to real lists), slide titles, slide reading order (PowerPoint — text stacked bottom-before-top in the shape tree, 1.3.2), document title &amp; language, link text (incl. bare-URL link text, 2.4.4), colour contrast (WCAG 1.4.3 — explicit, theme &amp; PDF text colours, vs. white), form-field labels (3.3.2 / 4.1.2 — PDF &amp; Word), untagged-PDF detection (1.3.1 / PDF-UA — remediation reconstructs the structure tree), and scanned PDF / no-extractable-text detection (1.1.1 — OCR required before remediation).<br>
        <strong>PDF note:</strong> untagged PDFs are flagged whole and the remediated file receives a reconstructed structure tree (headings, lists, tables, figures, artifacts, links, form fields). Tagged PDFs are analysed through their existing structure tree: headings (incl. level jumps), header-less tagged tables, and figure alt text are read from the tags themselves (RoleMap-aware).<br>
        <strong>Not evaluated — require manual review:</strong> colour contrast for inherited colours &amp; coloured-background pages, reading order beyond the PowerPoint stacked-shapes case (1.3.2), document parsing (4.1.1), and other success criteria.<br>
    ...
    <div class="footer">
      ${brandingFooterHtml(branding)} · State of the audit at ${today.toLocaleString()}.
    </div>
  </div>
</body></html>`;
}
...
function _buildConformanceReportHtml(opts: {
  ...
  const branding = loadBranding(); // white-label: agency logo/name/colour on the deliverable
...
<body>
  ...
  <div class="page">
    ...
    <div class="footer">${brandingFooterHtml(branding)} · Accessibility assessment · ${_escape(today)}</div>
  </div>
</body>
</html>`;
}
