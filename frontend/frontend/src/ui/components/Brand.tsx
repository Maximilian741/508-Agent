...
export function BrandMark({ size = 28, color }: BrandMarkProps) {
  ...
  if (Platform.OS === "web") {
    // Solid single-colour shield — no gradient. A flat mark reads as a
    // deliberate logo, not a generated "gradient blob".
    const svg = `
 ...
 */
export function favicon(accent = "#C2410C"): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path d="M16 1.5 L28.5 6 V16 C28.5 23.5 23 28.5 16 30.5 C9 28.5 3.5 23.5 3.5 16 V6 Z" fill="${accent}"/><path d="M11 22 L13.5 24.5 L20 18.5" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>`;
}
